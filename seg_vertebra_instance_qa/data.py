...
class DataGenerator_4_classes(tf.keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        with open('batch_shape_log.txt', 'a') as f:
            f.write(str(self.dim) + ', ' + str(X[:,:,:,:,0].astype(np.float64).sum()) + ', ' + str(X[:,:,:,:,0].astype(np.float64).sum()/X[:,:,:,:,0].astype(np.float64).size) + ', ' + str(X[:,:,:,:,0].min()) + ', ' + str(X[:,:,:,:,0].max())+ '\n')
        return X, tf.keras.utils.to_categorical(y, num_classes=self.n_classes)
...
class DataGenerator_4_classes_weights(tf.keras.utils.Sequence):
    'Generates data for Keras'
    ...
    def __load(self, file_name, file_name_bbox_ct_name, ID):
        ...
        if not pd.isna(label):
            ...
            assert (np.unique(arr) == np.array([0,
                                                1])).all(), f'Segmentation Mask have labels 0 and 1, but {np.unique(arr)}. {file_name}, {file_name_bbox_ct_name}, {ID}'

        assert np.unique(
            arr).size == 2, f'Segmentation Mask have not two labels, but {np.unique(arr)}. {file_name}, {file_name_bbox_ct_name}, {ID}'
